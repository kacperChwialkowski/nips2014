...
\end{equation}
where $N^m$ is a Cartesian power of a set $N= \{1,...,n\}$. For simplicity, we will often drop the second argument and write simply $V(h)$. We will also denote the tuple $(i_1,...,i_m)$ by $i$. %i.e. $\sum_{i \in N^m} f(\cdot) \equiv \sum_{(i_1,...,i_m) \in N^m} f()$. 
...
%Hilbert-Schmidt Independence Criterion (HSIC) \cite{gretton_measuring_2005,gretton_kernel_2008}, which can be interpreted as the distance between embeddings  of the joint distribution and the product of the marginals in a reproducing kernel Hilbert space (RKHS) \cite[Section 7]{gretton_kernel_2012}.  
...
% Let $i_{max}$ be a maximal element of a index set $\{i_1,...,i_{m}\}$. Using the fact that $h$ is Lipschitz continuous and canonical we derive 
% \begin{align}
%  & \frac{1}{n^{m-1}} \sum_{i \in N^{m}} |\ev   h(Z_{i_{max}},...)|\leq  \\
%  &\frac{1}{n^{m-1}} \sum_{i \in N^{m}}   |\ev   \left[h(Z_{i_{max}},...) - h(Z_{i_{max}}^*,...)\right] + \ev h(Z_{i_{max}}^*,...) | = \\
%  &  \frac{1}{n^{m-1}} \sum_{i \in N^{m}} |\ev   \left[h(Z_{i_{max}},...) - h(Z_{i_{max}}^*,...)\right] |  \leq \\
%  &  Lip(h) \frac{1}{n^{m-1}} \sum_{i \in N^{m}}  \ev|Z_{i_{max}} - Z_{i_{max}}^*| \leq  Lip(h) \frac{1}{n^{m-1}} \sum_{i \in N^{m}} \tau(i_{max}).
% \end{align}
% $\ev h(Z_{i_{max}}^*,...)=0$ follows from $h$ degeneracy. Inequality  
% \begin{align}
%  \ev|Z_{i_{max}} - Z_{i_{max}}^*| \leq \tau(i_{max}).
% \end{align}
% is a consequence of $\tau$-mixing properties from \cite{leucht_dependent_2013} (TODO maybe more precise). Finally 
% \begin{align}
% \lim_{n \to \infty} Lip(h) \frac{1}{n^{m-1}} \sum_{i \in N^{m}} \tau(i_{max}) \leq  Lip(h) \lim_{n \to \infty} \frac{1}{n^{m-1}} \sum_{i \in N^{m}} i_{max}^{-3} = 0.
% \end{align}
% To see that we change summing order to sum over $m$ dimensional dices for each $1 \leq i \leq n$ (which is already a rough approximation, since we could have summed over 'edges' of those dices )
% \begin{align}
% \sum_{i \in N^{m}} i_{max}^{-3} \leq  \sum_{1 \leq i \leq n} i^{-3} i^m =\sum_{1 \leq i \leq n} i^{m-3} \leq C n^{m-2}.
% \end{align}
%\end{proof}
...
% Let $i_{max}$ be a maximal element of a index set $\{i_1,...,i_{2m}\}$. We may assume that index $i_{max}$ is encountered in the first instance of $h$. Using the fact that the $h$ is bounded by $C_1$, Lipschitz continuous and canonical we derive 
% \begin{align}
%  &  \frac{1}{n^{2m-2}} \sum_{i \in N^{2m}} | \ev h(Z_{i_{max}},...)h(...) | = \\
%  &  \frac{1}{n^{2m-2}} \sum_{i \in N^{2m}} | \ev \left[h(Z_{i_{max}},...)h(...) - h(Z_{i_{max}}^*,...)h(...)\right] + \ev h(Z_{max}^*,...)h(...)| = \\
%  &  \frac{1}{n^{2m-2}} \sum_{i \in N^{2m}} | \ev \left[(h(Z_{i_{max}},...)h(...) - h(Z_{i_{max}}^*,...)h(...)\right] +0| \leq \\
%  &  \frac{1}{n^{2m-2}} \sum_{i \in N^{2m}} C_1| \ev \left[h(Z_{i_{max}},...) - h(Z_{i_{max}}^*,...)\right]|  \leq \\
%  &  \frac{1}{n^{2m-2}} C_1 \sum_{i \in N^{2m}} Lip(h) \ev |Z_{i_{max}} - Z_{i_{max}}^*| \leq   \frac{1}{n^{2m-2}} C_1 Lip(h) \sum_{i \in N^{2m}} \tau(i_{max}).
% \end{align}
% The thesis of this lemma follows form   
% \begin{align}
%  \frac{1}{n^{2m-2}} \sum_{i \in N^{2m}} \tau(i_{max}) \leq 2m \frac{1}{n^{2m-2}} \sum_{1 \leq  i \leq n }  i^{-3} i^{2m-1}  = O( n^{-1}).
% \end{align}
% The first inequality follows from the fact that there exists less than $ 2m i^{2m-1}$ tuples with an maximal element equal to $i$ and the mixing coefficient is such that  $\tau(i) = o(i^{-3})$. 
\end{proof}
